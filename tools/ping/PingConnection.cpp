...
PingConnection::ProcessEvent(
    ...
    case QUIC_CONNECTION_EVENT_DATAGRAM_SEND_COMPLETE: {
        auto SendRequest = (PingSendRequest*)Event->DATAGRAM_SEND_COMPLETE.ClientContext;
        if (!Event->DATAGRAM_SEND_COMPLETE.Cancelled) {
    ...
    case QUIC_CONNECTION_EVENT_DATAGRAM_ACKNOWLEDGED: {
        if (Event->DATAGRAM_ACKNOWLEDGED.Lost) {
            DatagramsLost++;
        } else {
            DatagramsAcked++;
        }
