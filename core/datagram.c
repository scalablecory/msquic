...
QuicDatagramCompleteSend(
    ...
    _In_ BOOLEAN Cancelled,
    _Out_opt_ void** AckContext
    )
{
    QUIC_CONNECTION_EVENT Event;
    Event.Type = QUIC_CONNECTION_EVENT_DATAGRAM_SEND_COMPLETE;
    Event.DATAGRAM_SEND_COMPLETE.AckContext = NULL;
    Event.DATAGRAM_SEND_COMPLETE.Cancelled = Cancelled;
    Event.DATAGRAM_SEND_COMPLETE.ClientContext = SendRequest->ClientContext;

    if (Cancelled) {
        QuicTraceLogConnVerbose(DatagramSendCancelled, Connection, "[dgrm][%p] Indicating DATAGRAM_SEND_COMPLETE [%p] Cancelled", SendRequest);
    } else {
        QuicTraceLogConnVerbose(DatagramSendComplete, Connection, "[dgrm][%p] Indicating DATAGRAM_SEND_COMPLETE [%p]", SendRequest);
    }
    ...
    if (!Cancelled) {
        *AckContext = Event.DATAGRAM_SEND_COMPLETE.AckContext;
    }
}
    ...
    )
{
    ...
    while (Datagram->SendQueue != NULL) {
        ...
        QuicDatagramCompleteSend(Connection, SendRequest, TRUE, NULL);
    }
    Datagram->PrioritySendQueueTail = &Datagram->SendQueue;
    Datagram->SendQueueTail = &Datagram->SendQueue;
    ...
    )
{
    ...
    QuicTraceLogConnVerbose(IndicateDatagramMaxLengthChanged, Connection, "[dgrm][%p] Indicating DATAGRAM_MAX_LENGTH_CHANGED [%hu]", NewMaxLength);
    (void)QuicConnIndicateEvent(Connection, &Event);
    ...
    )
{
    ...
    while (ApiQueue != NULL) {
        ...
        QuicTraceLogConnVerbose(DatagramSendQueued, Connection,
            "[dgrm][%p] Datagram [%p] queued with %llu bytes (flags 0x%x)",
    ...
    )
{
    ...
    while (Datagram->SendQueue != NULL) {
        ...
        if (HadRoomForDatagram) {
            ...
            QuicDatagramCompleteSend(Connection, SendRequest, FALSE,
                (void**)&Builder->Metadata->Frames[Builder->Metadata->FrameCount].DATAGRAM.AckContext);
    ...
    )
{
    ...
    QuicTraceLogConnVerbose(IndicateDatagramReceived, Connection,
        "[dgrm][%p] Indicating DATAGRAM_RECEIVED [len=%hu]", (uint16_t)Frame.Length);
    (void)QuicConnIndicateEvent(Connection, &Event);
...
{
    QUIC_CONNECTION_EVENT Event;
    Event.Type = QUIC_CONNECTION_EVENT_DATAGRAM_ACKNOWLEDGED;
    Event.DATAGRAM_ACKNOWLEDGED.ClientContext = FrameMetadata->DATAGRAM.ClientContext;
    Event.DATAGRAM_ACKNOWLEDGED.AckContext = FrameMetadata->DATAGRAM.AckContext;
    Event.DATAGRAM_ACKNOWLEDGED.Lost = TRUE;

    QUIC_CONNECTION* Connection = QuicDatagramGetConnection(Datagram);
    QuicTraceLogConnVerbose(IndicateDatagramLost, Connection,
        "[dgrm][%p] Indicating QUIC_CONNECTION_EVENT_DATAGRAM_ACKNOWLEDGED [ClientContext=%p, AckContext=%p, Lost]",
        FrameMetadata->DATAGRAM.ClientContext, FrameMetadata->DATAGRAM.AckContext);
    (void)QuicConnIndicateEvent(Connection, &Event);
}
    ...
    )
{
    ...
    Event.DATAGRAM_ACKNOWLEDGED.ClientContext = FrameMetadata->DATAGRAM.ClientContext;
    Event.DATAGRAM_ACKNOWLEDGED.AckContext = FrameMetadata->DATAGRAM.AckContext;
    Event.DATAGRAM_ACKNOWLEDGED.Lost = FALSE;

    QUIC_CONNECTION* Connection = QuicDatagramGetConnection(Datagram);
    QuicTraceLogConnVerbose(IndicateDatagramAcknowledged, Connection,
        "[dgrm][%p] Indicating QUIC_CONNECTION_EVENT_DATAGRAM_ACKNOWLEDGED [ClientContext=%p, AckContext=%p, Acked]",
        FrameMetadata->DATAGRAM.ClientContext, FrameMetadata->DATAGRAM.AckContext);
    (void)QuicConnIndicateEvent(Connection, &Event);
}
